      ...
      */
      ...
      /*
        **손에 든 가로 화면은 조금 당겨 본다** (사용자가 정했다: *"가로모드여서 줌인을 조금 더 해도 전체가
        ...
        PC(98°)보다 훨씬 넓어 멀리 있는 신호등과 사람이 그만큼 작다. 세로 화각을 1/1.3 으로 조이면 가로
        약 100° 가 되어 교차로 양 끝은 그대로 들어오면서 멀리 있는 것이 30% 커진다. PC · 세로는 그대로다.
      */
...
/** 손에 든 가로 화면의 후방 시점을 당겨 보는 배율 — 세로 화각을 이만큼 나눈다 (위 chase 주석). 1.2 로 두었다가 "조금만 더" 로 1.3 */
const CHASE_ZOOM_LANDSCAPE = 1.3;
/**

 ...
 *
 * 12판이던 것을 20판으로 넓혔다. 40판을 이어 달려 재 보니 서로 다른 판이 26판뿐이었는데(같은 판을 다시
 * 만나기까지 열두 판이면 한 번 앉은 동안에도 되풀이가 눈에 띈다), 20판으로 넓히니 31판이 됐다. 후보는
 * 한 레벨에 수백 판이고(가장 적은 L1 도 163판) 비어도 아래 티어로 내려가므로, 넓혀도 빈손이 되지 않는다.
 */
  ...
  */
  ...
  /*
    **보호구역 전용 갈래는 다른 판 묶음에서 고른다** (scenarios/zoneCourse.ts) — 우회전 없이 보호구역을
    직진으로 통과하는 156판이다. 기존 라이브러리에는 그런 판이 하나도 없으므로 거르는 것이 아니라 **갈아 끼운다.**
  */
  const all =

 ...
 *
 * 처음 곡선은 L1~L3 이 100 이라 무위반 한 판이면 올랐다(1 · 1 · 1 · 2 · 3 · 4 · 5 · 6 · 8 · 10, 모두 41판). 사용자가
 * 짚었다 — "1판만 했는데 레벨 2로 올라갔어 … 저랩에서 너무 쉽게 레벨이 올라가는 것 같아."
 *
 * 한 판은 운일 수 있다. 보행자가 마침 안 나왔거나 신호가 마침 맞았을 수 있다. 그 레벨의 개념(L2 보호구역, L3 신호기 없는
 * 횡단보도 …)을 **서로 다른 맵 셋**에서 지켜 내야 익혔다고 할 수 있다 — 그래서 가장 낮은 레벨도 세 판이다. 그 위로는 한
 * 레벨 걸러 한 판씩 늘어, 게임처럼 오를수록 오래 머문다. 경험치는 넘기지 않으므로(아래 advance) 세 판은 줄지 않는다 —
 * 습관을 고쳐 받는 +50 이 한 판을 줄여 줄 수는 있다.
 */
...
export const xpToNext = (level: Difficulty, rule: ProgressRule = DEFAULT_PROGRESS): number =>
  Math.max(50, Math.round((XP_TO_NEXT[level] * rule.xpScale) / 50) * 50);
 ...
 * 우회전 신호등 → L2 어린이보호구역 → L3 신호기 없는 보호구역 → L5 앞차 · 꼬리물기 → L7 일시정지 무시 앞차 · 보행자
 ...
 * 가장 낮은 레벨도 무위반 세 판을 머물고, 위로 갈수록 오래 머문다 (XP_TO_NEXT).
 *
 ...
 * "이 과정에서 몇 판을 풀었나" 라 첫 화면의 버튼 글자(`운행 시작` / `이어서 운행`)를

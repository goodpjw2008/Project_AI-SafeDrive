...
export interface PedSpawn {
  ...
  letsCarPass?: boolean;
  /**
   * **자전거를 가진 사람** — 사용자가 실제 도로 사진을 주며 넣자고 했다: "횡단보도 끝에 저렇게 나와 있는
   * 곳은 자전거를 타고 통행이 가능해. 반대로 하얀색 선에서는 자전거를 끌고 가야 해."
   *
   *  - `push` **끌고 건넌다** — 자전거횡단도가 없는 횡단보도에서 법이 시키는 모습이다(제13조의2 제6항).
   *    끌고 가는 사람은 **보행자**이므로(제2조 제17호) 판정도 보행자 그대로다. 걸음도 사람 걸음이다.
   *  - `ride` **타고 건넌다** — 걸음의 **두 배 넘게 빠르다.** 멀리 있다고 먼저 지나가면 닿는다.
   *    자전거횡단도(`ScenarioSpec.bikeLane`)가 있는 자리면 제15조의2 제3항의 일시정지 대상이고,
   *    없는 자리면 그 사람은 보행자가 아니다 — 그래도 안전운전의무(제48조)는 그대로다.
   */
  bike?: 'ride' | 'push';
}
...
export interface ScenarioSpec {
  ...
  /**
   * **자전거횡단도가 설치된 횡단보도** — 없으면 생략한다.
   *
   * 실물은 횡단보도 **옆에 붙은 붉은 포장 띠**이고 가운데에 자전거 표시가 그려져 있다
   * (game/Intersection.ts 의 `drawBikeLane`). 거기서는 자전거를 **타고** 건널 수 있고,
   * 차의 운전자는 그 앞에서 일시정지해야 한다 (제15조의2 제3항).
   *
   * **표시가 없으면 타고 건널 수 없다** — 내려서 끌고 건너야 하고(제13조의2 제6항), 그렇게
   * 끌고 가는 사람은 보행자다. 그래서 이 값은 **노면에 그리는 것이자 판정의 근거**다:
   * 자전거가 타고 건널 때 `BIKE_BLOCKED` 가 어느 조문으로 적히는지를 이 값이 가른다.
   */

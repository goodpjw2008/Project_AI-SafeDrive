...
> **참고**: 본 프로젝트의 자체 작성 코드 · 로봇 캐릭터 그림 · README 의 글과 그림은 **모든 권리 보유**이며, 사용하려면 먼저 허락을 받아야 합니다 ([LICENSE](LICENSE)). 아래의 제3자 자산은 **각 원작자의 라이선스**를 그대로 따르며, LICENSE 가 무엇을 정하든 그 조건대로 써야 합니다.
>
> 3D 자동차 모델 중 4종(아반떼 N · M8 · M5 · SL63)은 **비영리(NC)** 조건입니다. 이 모델이 들어 있는 배포 사이트(https://safelevelup.vercel.app)는 비영리로만 운영합니다. 이 저장소를 상업적으로 쓰려면 해당 모델 파일과 그 차 사진을 빼야 합니다.

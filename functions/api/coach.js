/**
 * 배포용 `/api/coach` — Cloudflare Pages Functions.
 *
 ...
 * ## 올리는 법
 *
 * `dist/` 를 Cloudflare Pages 에 올리면 이 폴더가 함께 배포되어 `/api/coach` 가 열린다.
 ...
 * `/api/coach` 경로에 IP당 분당 몇 회 정도로 걸어 둔다.
 * (입력 크기 제한은 server/coachHandler.mjs 가 이미 하고 있다 — 남용 시도는 그쪽에서 걸린다)
 *
 * Netlify·Vercel 로 간다면 이 파일만 각자의 함수 규약에 맞게 바꾸면 된다
 * (`handleCoach` 를 부르는 부분은 그대로다).
 */
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    /* handleCoach 가 BAD_INPUT 으로 답한다 */
  }

  const { status, body } = await handleCoach(parsed, env);

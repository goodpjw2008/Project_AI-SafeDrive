...
export function sanitize(body) {
  ...
  return {
    ...
    */
    ...
    badHabits: (Array.isArray(body.badHabits) ? body.badHabits : []).slice(0, LIMIT.habits).map((h) => ({
 ...
 */
export function parseReply(text, ids) {
  let raw;
  try {
    raw = JSON.parse(text);
  } catch {
...
export async function handleRecommend(body, env) {
  ...
  const reply = parseReply(res.body.text, req.courses.map((c) => c.id));

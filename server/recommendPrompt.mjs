...
/** 후보의 표 (src/ai/outcome.ts 의 LABEL_TEXT 와 같다) */
const LABEL_TEXT = { weak: '약점시험', zpd: '근접발달', review: '복습', novel: '새로움' };
export const SYSTEM_PROMPT = [
  ...
  '- `예상위반:보행자양보42%` 는 결과 예측 모델이 **이 학습자가 이 코스에서 그 개념을 어길 확률**을 센 것입니다.',
  '  약한 개념을 고치는 코스라면 **30~70%** 가 가장 많이 배우는 자리입니다 — 90% 는 아직 이르고, 5% 는 배울 게 없습니다.',
  '- `표:약점시험,근접발달,복습,새로움` 은 분류 모델이 후보에 붙인 표입니다. 표가 많은 코스가 대개 맞지만, 표는 후보를',
  '  추린 근거이지 결론이 아닙니다 — 최근 주행과 습관을 읽고 그 가운데 **지금 이 사람에게** 맞는 하나를 고르십시오.',
  ...
  '',
  '## 고르는 원칙',
  '1. **먼저 고칠 습관**을 봅니다. 습관이 하나면 그 습관을 시험하는 코스를 고릅니다.',
  '   요청에 **"먼저 고칠 습관을 정하십시오"** 가 있으면 여러 습관 중 **지금 가장 먼저 고쳐야 할 것**을 직접 정합니다.',
  '   많이 한 순서만 보지 않습니다 — 최근 주행에서 **이어 나오는가**, 그 지점의 **준수율**, 풀리기까지 **남은 판 수**,',
  '   **사람이 다칠 수 있는가**(보행자 · 어린이가 걸린 위반이 방향지시등 같은 조작 실수보다 급합니다)를 함께 봅니다.',
  '   그리고 그 습관의 묶음(후보 줄 앞 `[코드]`)에서 코스를 고릅니다. 지점별 준수율이 낮은 곳과 이어지는 코스면 더 좋습니다.',
  '2. **최근 주행을 봅니다.**',
  ...
  '{"habit": "위반 코드", "id": 후보 번호, "why": "…", "focus": "…"}',
  '- `habit`: **먼저 고칠 습관을 정하라고 했을 때만** — 정한 습관의 위반 코드(나쁜 운전 습관 목록에 있는 것). 아니면 뺍니다.',
  '- `id`: **반드시 후보 목록에 있는 번호**. habit 을 적었다면 그 습관의 묶음에 있는 번호.',
  ...
  '  먼저 고칠 습관을 정했다면 **왜 그 습관이 먼저인지**를 기록의 사실로 말합니다.',
  '  **기록에 없는 사실은 쓰지 않습니다** — 위반 코드와 숫자는 위 기록에 있는 것만 씁니다. 기록이 없으면 없다고 말합니다.',
  '  **영문 위반 코드(`RED_NO_STOP` 같은 것)나 `난N` · `[코드]` 같은 후보 표기는 쓰지 않습니다** — 학습자가 읽는 글이므로',
  '  습관 이름과 상황을 한국어로 씁니다.',
...
function courseLine(c) {
  ...
  // 결과 예측 — 어길 확률이 높은 개념 · 후보에 붙은 표 (src/ai/outcome.ts)
  if (c.predict?.length) bits.push(`예상위반:${c.predict.map((x) => `${SKILL_TEXT[x.code] ?? x.code}${x.p}%`).join(',')}`);
  if (c.labels?.length) bits.push(`표:${c.labels.map((l) => LABEL_TEXT[l] ?? l).join(',')}`);
...
export function buildUserPrompt(req) {
  ...
  if (req.badHabits.length) {
    ...
    lines.push(
      req.priority
        ? `  일어날 수 있는 코스에서 ${HABIT_CLEARED_AFTER}번 지켜야 풀립니다. **먼저 고칠 습관을 정하십시오** — 후보는 습관마다 따로 추렸고, 줄 앞 \`[코드]\` 가 그 코스가 고치는 습관입니다.`
        : `  일어날 수 있는 코스에서 ${HABIT_CLEARED_AFTER}번 지켜야 풀립니다. 후보는 모두 첫째 습관을 시험하는 코스입니다.`,
    );
